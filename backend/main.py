...
def compute_phash(image: np.ndarray) -> imagehash.ImageHash:
    rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    return imagehash.phash(imagehash.Image.fromarray(rgb))
...
class HostEngineIdentifier:
    ...
    def identify(self, image_bytes: bytes, mode: str) -> Optional[dict]:
        ...
        query_hash = compute_phash(crop)

        hash_distances = [
            (index, query_hash - imagehash.hex_to_hash(phash))
            for index, phash in enumerate(self.brain.card_phash)
        ]
        hash_distances.sort(key=lambda item: item[1])
        phash_candidates = [index for index, _ in hash_distances[:PHASH_CANDIDATES]]

        candidate_indices = self._stage_one_candidates(hsv_hist, use_deck=True)
        if phash_candidates:
            candidate_indices = [idx for idx in candidate_indices if idx in phash_candidates]
        ...
        if phash_candidates and candidate_indices:
            candidate_indices = [idx for idx in candidate_indices if idx in phash_candidates]
        ...
        if result:
            ...
            if result["confidence"] == "Low":
                log_time(f"Identify latency: {time.time() - start_time:.3f}s")
